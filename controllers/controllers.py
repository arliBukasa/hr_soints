...
class HrSoints(http.Controller):
    ...
    def employee_info(self, qr_code):
        # Recherche de l'employé correspondant au QR code
        employee = request.env['hr.employee'].sudo().search([('qr_code', '=', qr_code)], limit=1)
        logging.info(f"*********************************************** employee: {employee}, Qrcode : {qr_code} , employee sans filtre : {request.env['hr.employee'].sudo().search([])}")
        
        if not employee:
            return request.render('hr_soints.employee_not_found')
        
        # Rendre la vue publique avec les informations de l'employé
        return request.render('hr_soints.employee_info', {
            'employee': employee
        })
